...
#define MAX_WATER_DISTANCE 4
#define MIN_WATER_DISTANCE 12

// Constants
#define HOUR_INTERVAL_MS 3600000 // 1 hour in milliseconds
#define DAY_INTERVAL_MS 86400000 // 1 day in milliseconds
...
std::deque<float> hourlyData; // To store water_percent for the last hour
std::deque<float> dailyData;  // To store water_percent for the last day
unsigned long lastHourUpdate = 0;
unsigned long lastDayUpdate = 0;
float last_hour_percent = 0;
float last_day_percent = 0;
...
void SensorTaskcode( void * pvParameters ){
  for(;;){
    if (xSemaphoreTake(xSemaphore, (TickType_t)10) == pdTRUE) {
      ...
      // // Add current water_percent to the tracking queues
      // if (currentMillis - lastHourUpdate >= HOUR_INTERVAL_MS / 12) { // Store every 5 minutes
      //   hourlyData.push_back(water_percent);
      //   if (hourlyData.size() > 12) hourlyData.pop_front(); // Keep the last hour's data
      //   lastHourUpdate = currentMillis;
      // }

      // if (currentMillis - lastDayUpdate >= DAY_INTERVAL_MS / 144) { // Store every 10 minutes
      //   dailyData.push_back(water_percent);
      //   if (dailyData.size() > 144) dailyData.pop_front(); // Keep the last day's data
      //   lastDayUpdate = currentMillis;
      // }

      // // Calculate averages and update Firebase
      // if (currentMillis - lastHourUpdate >= HOUR_INTERVAL_MS) {
      //   last_hour_percent = calculateAverage(hourlyData);
      // }

      // if (currentMillis - lastDayUpdate >= DAY_INTERVAL_MS) {
      //   last_day_percent = calculateAverage(dailyData);
      // }
  ...
  for(;;){
    if (xSemaphoreTake(xSemaphore, (TickType_t)10) == pdTRUE) {
      ...
      if (app.ready() && millis() - tmo > 1000) {
        ...
        doc["lastHourPercent"] = 50;
        doc["lastDayPercent"] = 50;
...
float calculateAverage(std::deque<float> &data) {
  float sum = 0;
  for (float value : data) {
    sum += value;
  }
  return data.empty() ? 0 : sum / 12;
}